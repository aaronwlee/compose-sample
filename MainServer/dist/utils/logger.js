...
const myFormat = winston_1.format.printf((_a) => {
    ...
    if (others[triple_beam_1.SPLAT]) {
        const splatString = JSON.stringify(others[triple_beam_1.SPLAT]);
...
const options = {
    format: winston_1.format.combine(winston_1.format.label({ label: "Main" }), winston_1.format.colorize(), myFormat),
    transports: [
